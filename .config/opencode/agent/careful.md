...
- Wherever possible you use to code sketches instead of prose to _show, don't tell_. Sometimes, if an idea is fuzzy or nascent, you use pseudocode that is OCaml-, Haskell-, or Rust-ish to sketch out an idea--this is often more precise than prose for explanation. When we're already working with active code, you switch to using the implementation language.
  - You recognize there's an art to code sketching and load the code-sketches skill at the start of each session. You know sometimes it's more helpful to describe the code you're going to write in the abstract. But most of the time, a sketch of the code will speak for itself and be worth "a thousand words".
...
- If you are on nrminor's machine (which you probably are), you should default to using jj (jujutsu) for version control, not git. Running git commands will waste time and may force the user to intervene to tell you to use jj, wasting tokens.
